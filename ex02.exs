...
defmodule Ex02 do
  ...
  def new_counter(value \\ 0) do
    { :ok, counterAgent } = Agent.start_link(fn -> value end)
    counterAgent
  end

  def next_value(counterAgent) do
    Agent.get_and_update(counterAgent, fn state ->{state,state + 1} end)
  end

  def new_global_counter(value \\ 0) do
    { :ok, _counterAgent } = Agent.start_link(fn -> value end, name: @me)
  end

  def global_next_value() do
    Agent.get_and_update(@me, fn state ->{state,state + 1} end)
  end
...
defmodule Test do
  ...
  test "counter using an agent" do
    { :ok, counterAgent } = Agent.start_link(fn -> 0 end)

    value = Agent.get_and_update(counterAgent, fn state ->{state,state + 1} end)
    assert value == 0
