...
defmodule Ex02 do
    def new_counter(value \\ 0) do
      { :ok, agentCouter } = Agent.start_link(fn -> value end)
      agentCouter
    end

    def next_value(agentCouter) do
      Agent.get_and_update(agentCouter, fn state ->{state,state + 1} end)
    end

    def new_global_counter(value \\ 0) do
      { :ok, _agentCouter } = Agent.start_link(fn -> value end, name: __MODULE__)
    end
