...
#######
  ...
  def new_global_counter do
    status_pid = Agent.start_link(fn -> 0 end, name: GCounter)
    {_status, _pid} = status_pid
  end

  def global_next_value do
    Agent.get_and_update(GCounter, @name, :counter, [ ] )
  end
  ...
  def next_value(agent) do
    {_status, pid} = agent
    Agent.get_and_update(pid, @name, :counter, [ ] )
...
defmodule Test do
  ...
  """
   test "counter using an agent" do
     ...
     value   = Agent.get(counter, fn count -> count end)
     assert value == 0

     value = Agent.get_and_update(counter, fn count -> {(count + 1), :OK} end)
     assert value == 1
   end
